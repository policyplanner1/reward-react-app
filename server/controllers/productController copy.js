...
class ProductController {
  ...
  async createProduct(req, res) {
    ...
    try {
      ...
      // 6  Handle variants
      if (body.variantOptions && body.skus) {
        const variantOptions =
          typeof body.variantOptions === "string"
            ? JSON.parse(body.variantOptions)
            : body.variantOptions;

        const skus =
          typeof body.skus === "string" ? JSON.parse(body.skus) : body.skus;

        // A. Insert Variant Options + Values
        const optionMap = {};
        // { Color: { optionId, values: { Red: valueId } } }

        for (const option of variantOptions) {
          const optionId = await ProductModel.insertVariantOption(
            connection,
            productId,
            option.name
          );
          ...
          for (const value of option.values) {
            const valueId = await ProductModel.insertVariantOptionValue(
              connection,
              optionId,
              value
            );
            optionMap[option.name].values[value] = valueId;
          }
        }
        ...
        for (let i = 0; i < skus.length; i++) {
          ...
          const variantId = await ProductModel.insertSKU(
            connection,
            productId,
            sku
          );

          // C. Map SKU → attributes
          for (const [optionName, optionValue] of Object.entries(
            sku.attributes
          )) {
            const optionData = optionMap[optionName];
            if (!optionData) continue;

            const optionId = optionData.optionId;
            const valueId = optionData.values[optionValue];
            if (!valueId) continue;

            await ProductModel.insertVariantAttribute(
              connection,
              variantId,
              optionId,
              valueId
          ...
          // D. SKU Images
          const skuImages = movedFiles.filter((f) =>
            f.fieldname.startsWith(`sku_${i}_`)
          );
