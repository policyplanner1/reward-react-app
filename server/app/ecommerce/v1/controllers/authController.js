...
async loginUser(req, res) {
  try {
    ...
    /*
      FIRST TIME LOGIN / OLD CUSTOMER:
      Agar customer.device_id blank hai to current device save karo.
    */
    if (!user.device_id) {
    ...
    /*
      NEW DEVICE LOGIN:
      Agar database wala device_id aur current device_id different hai,
      to login block hoga aur approval mail jayega.
      Important: JWT token yaha generate nahi hoga.
    */
    if (user.device_id && user.device_id !== currentDeviceId) {
      ...
      /*
        ...
        Same user ke old pending device requests remove karo,
        taaki multiple emails/old tokens ka confusion na ho.
      */
    ...
    /*
      SAME DEVICE LOGIN:
      Ab device verified hai, isliye token generate hoga.
    */
    const accessToken = jwt.sign(
      {
        user_id: user.user_id,
