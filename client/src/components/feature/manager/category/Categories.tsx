...
export default function CategoryManagement() {
  ...
  try {
    ...
    // Check if res.data.data exists or is already an array
  ...
  const handleAdd = async (e?: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      await api.post("/vendor/create-category", { name: newCategoryName, status: 1 });
      setNewCategoryName("");
      fetchCategories();
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen p-8 bg-[#FAFAFE]">
      ...
      <div className="mb-10">
        ...
        <p className="mt-2 ml-16 font-medium text-gray-400">Manage and organize your product catalog</p>
      </div>

      {/* ADD CATEGORY INPUT */}
      <form onSubmit={handleAdd} className="flex gap-4 p-2 mb-10 bg-white shadow-sm rounded-2xl border border-gray-100/50 max-w-3xl">
        <input
        ...
        <table className="min-w-full border-collapse">
          <thead>
            <tr className="border-b border-gray-50">
              <th className="px-8 py-6 text-xs font-black tracking-widest text-left text-gray-400 uppercase">Category Name</th>
          ...
          <tbody className="divide-y divide-gray-50">
            {categories.map(cat => (
              <tr key={cat.category_id} className="group transition-colors hover:bg-gray-50/50">
                <td className="px-8 py-5 text-sm font-bold text-gray-700">{cat.name}</td>
                <td className="px-8 py-5">
                  <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-[11px] font-black uppercase tracking-wider ${
                    ...
                    <span className={`w-1.5 h-1.5 rounded-full ${cat.status === "active" ? "bg-emerald-500" : "bg-red-500"}`} />
                    {cat.status}
                  </span>
                </td>
                ...
                <td className="px-8 py-5">
                  <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => handleView(cat.category_id)} className="p-2.5 text-gray-400 hover:text-[#852BAF] bg-white border border-gray-100 rounded-xl shadow-sm hover:shadow-md transition-all">
                      <FiEye size={16} />
                    </button>
                    <button onClick={() => { handleView(cat.category_id); setIsEditing(true); }} className="p-2.5 text-gray-400 hover:text-[#FC3F78] bg-white border border-gray-100 rounded-xl shadow-sm hover:shadow-md transition-all">
                      <FiEdit size={16} />
                    </button>
                    <button onClick={() => handleDelete(cat.category_id)} className="p-2.5 text-gray-400 hover:text-red-500 bg-white border border-gray-100 rounded-xl shadow-sm hover:shadow-md transition-all">
                      <FiTrash2 size={16} />
                    </button>
                  </div>
                </td>
        ...
        <div className={`fixed inset-0 z-50 transition-all duration-500 ${drawerOpen ? "visible" : "invisible"}`}>
          ...
          <div className={`absolute right-0 top-0 h-full w-[450px] bg-white shadow-[-20px_0_50px_rgba(0,0,0,0.05)] transition-transform duration-500 ease-out ${
            ...
            <div className="p-8 border-b border-gray-50">
              ...
              <h2 className="text-2xl font-black text-gray-900">{isEditing ? "Edit Category" : selected.name}</h2>
              ...
              {!isEditing ? (
                <div className="space-y-6">
                  <div className="p-6 bg-gray-50 rounded-[2rem] border border-gray-100">
                    ...
                    <p className="text-lg font-bold text-gray-800 capitalize">{selected.status}</p>
                  </div>
                  <button
                ...
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
                  <div>
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-2 mb-2 block">Category Name</label>
                    <input
                      value={editName}
                  ...
                  <div>
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-2 mb-2 block">Display Status</label>
                    <select 
